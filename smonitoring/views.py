...
def index(request):
	...
	if filtre != None and filtre != 'all':
		...
		with connection.cursor() as cursor:
			cursor.execute('SELECT COUNT(*) qte,code_site_id,nom from smonitoring_evenement e,smonitoring_site s WHERE e.code_site_id=s.id AND status_ev="down" AND s.region=%s GROUP BY code_site_id ORDER BY qte DESC LIMIT 5',params=[filtre.upper()])
