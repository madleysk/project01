...
def index(request):
	...
	if filtre != None and filtre != 'all':
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT COUNT(*) as qte,code_site_id,nom from smonitoring_evenement e, smonitoring_site s WHERE (e.code_site_id=s.id) AND (e.status_ev=%s) AND (s.region=%s) GROUP BY e.code_site_id ORDER BY qte DESC LIMIT 5", params=['down',filtre.upper()])
		...
		with connection.cursor() as cursor:
			cursor.execute("SELECT COUNT(*) as qte,code_site_id,nom from smonitoring_evenement e,smonitoring_site s WHERE e.code_site_id=s.id AND e.status_ev=%s GROUP BY code_site_id ORDER BY qte DESC LIMIT 5",params=['down'])
			top_bad_sites = cursor.fetchall()
			top_bad_sites_formated = []
			for t in top_bad_sites:
				top_bad_sites_formated.append({"qte":t[0],"id":t[1],"nom":t[2]})
